...
const INITIAL_ANSWER: FeedbackAnswer = { isUseful: null, reasons: [], notes: "", gender: "" };
...
export default function Feedback() {
  ...
  const currentOptions = useMemo(() => (answer.isUseful ? OPTIONS_MAP[answer.isUseful] : []), [answer.isUseful]);
        ...
        <section className="feedback-section" aria-label="تقييم الصفحة">
          ...
          <div className="feedback-row">
            ...
            <div className="feedback-left">
              {submitted ? (
                <p className="text-md-regular" role="status" aria-live="polite"
                  style={{ display: "flex", alignItems: "center", gap: 16, color: "#161616" }}>
                  <Image src="/assets/icons/stroke-standard/checkmark-circle-04-stroke-rounded.svg"
                    alt="" aria-hidden="true" width={24} height={24} className="green-icon" />
                  تم إرسال ملاحظاتك!
                </p>
              ) : (
                <>
 ...
 <div role="group" aria-labelledby={questionId} className="feedback-actions">
          ...
          <div id={surveyPanelId} role="region" aria-label="نموذج التقييم التفصيلي"
            ...
            <div className="feedback-survey-panel__inner">
              <div className="feedback-form">
                {answer.isUseful && (
                  <>
                    {hasErrors && (
                      <Notification className="!mt-4" variant="critical" leadText="مهم"
                    ...
                    <div className="feedback-reasons-row">
                      <fieldset style={{ border: "none", padding: 0, margin: 0 }}
                        ...
                        <legend className="text-md-semibold"
                          style={{ color: "#161616", marginBottom: 16 }}>
                          يرجى إخبارنا بالسبب{" "}
                          <span className="text-sm-regular" style={{ color: "#6C737F" }}>
                            (يمكنك تحديد خيارات متعددة)
                          </span>
                        </legend>
                        <div className="feedback-checkboxes">
                          {currentOptions.map((option) => (
                            <DgaCheckbox key={option.id} label={option.text} value={option.id}
                              checked={answer.reasons.includes(option.id)}
                              onChange={() => handleReasonChange(option.id)} />
                          ))}
                          {errors.reasons && (
                            <div id={reasonsErrorId} role="alert" className="feedback-error">
                              <Image src="/assets/icons/stroke-standard/alert-circle-stroke-rounded.svg"
                                alt="" aria-hidden="true" width={16} height={16} className="icon-critical" />
                      ...
                      <Textarea ref={textareaRef} label="الملاحظات" name="notes"
                        value={answer.notes} scrollbar resize
                        onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) =>
                          setAnswer((prev) => ({ ...prev, notes: e.target.value }))}
                        variant="default" translate="yes" lang="ar" />
                    </div>

                    {/* Gender */}
                    <fieldset className="feedback-gender"
                      aria-describedby={errors.gender ? genderErrorId : undefined}>
                      <legend className="text-md-semibold"
                        style={{ color: "#161616", marginBottom: 8 }}>أنا</legend>
                      <div className="feedback-gender-options">
                        <DgaRadioButton name="gender" label="ذكر" value="male"
                          checked={answer.gender === "male"}
                          onChange={() => handleGenderChange("male")} />
                        <DgaRadioButton name="gender" label="أنثى" value="female"
                          checked={answer.gender === "female"}
                          onChange={() => handleGenderChange("female")} />
                      </div>
                      {errors.gender && (
                        <div id={genderErrorId} role="alert" className="feedback-error">
                          <Image src="/assets/icons/stroke-standard/alert-circle-stroke-rounded.svg"
                            alt="" aria-hidden="true" width={16} height={16} className="icon-critical" />
                          يرجى تحديد الجنس
                        </div>
