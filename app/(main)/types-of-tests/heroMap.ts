...
// Function to fetch dynamic hamza vocabulary test hero data
export async function getHamzaVocabularyTestHero(): Promise<HeroData & { breadcrumbs?: Crumb[] }> {
  try {
    const baseURL = process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000';
    ...
    if (!response.ok) {
      throw new Error('Failed to fetch hamza vocabulary test data');
    }
    
    const data = await response.json();
    
    return {
      title: data.header?.titleText || "اختبار همزة المفردات",
      description: data.header?.descriptionText || "",
      bgColor: "#FFF",
      externalLink: {
        href: "/sign-up",
      ...
      breadcrumbs: [
        ...
        { label: "عن الجهة", disabled: true },
        { label: "أنواع اختبارات همزة", path: "/types-of-tests" },
        { label: data.header?.titleText || "اختبار همزة المفردات", disabled: true },
      ],
    };
  } catch (error) {
    console.error('Error fetching hamza vocabulary test hero:', error);
    // Return static fallback data
    return {
      title: "اختبار همزة المفردات",
      description:
